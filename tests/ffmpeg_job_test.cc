...
static void test_convert_quicktime_to_flv()
{
  int status = 0;
  // load a job from the db
  WQ::DB::Adaptor::Iterator *it = adaptor->query_with_result("select id, name, data from jobs where name='ffmpeg' and status='pending' and locked_queue_id=0 limit %d", 1);
  check(it);
  if( it ) {
...
int main(int argc, char **argv)
{
  ...
  adaptor = &ma;
  GModule *module = g_module_open("modules/ffmpeg", G_MODULE_BIND_MASK);

  if( !module ) {
    g_error("%s: %s", "ffmpeg", g_module_error ());
    return 1;
  }
  ...
  run_tests("FFMPEG Job Test:", test_convert_quicktime_to_flv,  NULL);
